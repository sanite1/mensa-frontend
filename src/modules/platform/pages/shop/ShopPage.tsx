...
export function ShopPage() {
  ...
  const activeCategory: ProductCategory | null = isCategory(categoryParam)
    ? categoryParam
    : null

  // Fetch all products (no pagination at launch — 8 SKUs).
  const allProductsQuery = useProducts({ pageSize: 60 })
  const products = allProductsQuery.data?.data?.items ?? []
  ...
  return (
    <div className="bg-[var(--paper)]">
      ...
      {/* ── Filter bar ──────────────────────────────────────────────── */}
      <div className="border-y border-[var(--hairline)] bg-[var(--paper)]">
...
function PageHeader({ productCount }: { productCount: number }) {
  return (
    <section className="px-5 md:px-10 lg:px-16 pt-10 md:pt-16 lg:pt-[88px] pb-7 md:pb-10 lg:pb-14">
      {/* Breadcrumb */}
      <div className="flex items-center gap-3 font-sans text-[13px] text-[var(--mute)]">
      ...
      >
        Shop <span style={{ color: 'var(--pink)' }}>everything</span> Mensa.
      </h1>

      {/* Trust strip */}
      <div className="flex items-baseline justify-between gap-6 flex-wrap mt-6">
        <p
          className="m-0 text-[var(--graphite)] max-w-[620px]"
          style={{ fontSize: 'clamp(15px, 2vw, 18px)', lineHeight: 1.55 }}
        >
          {productCount > 0 ? (
            <>
        ...
        <div className="flex items-center gap-3.5 text-[13px] text-[var(--graphite)] flex-wrap">
    ...
    <div className="grid gap-3.5 md:gap-5 lg:gap-6 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="flex flex-col gap-3">
          <div
            className="w-full bg-[var(--cream-soft)] animate-pulse"
            style={{ aspectRatio: '4/5' }}
          />
          <div className="h-6 bg-[var(--cream-soft)] animate-pulse w-3/4" />
          <div className="h-3 bg-[var(--cream-soft)] animate-pulse w-1/2" />
        </div>
      ))}
    </div>
  )
...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="py-24 text-center">
      <div className="t-eyebrow text-[var(--err)] mb-4">Something went wrong</div>
      <h2
        className="m-0"
        style={{
          fontFamily: 'var(--font-display)',
      ...
      <button
        type="button"
        onClick={onRetry}
        className="mt-6 inline-flex items-center gap-2 text-[var(--ink)] text-[14px] font-medium border-b border-[var(--ink)] pb-0.5"
      >
...
function EmptyState({ onReset }: { onReset: () => void }) {
  return (
    <div className="py-24 text-center">
      <div className="t-eyebrow text-[var(--mute)] mb-4">No matches</div>
      <h2
        className="m-0"
        style={{
          fontFamily: 'var(--font-display)',
...
function GridFooter({ total, totalAll }: { total: number; totalAll: number }) {
  return (
    <div className="flex items-center justify-between pt-10 md:pt-14 gap-6 flex-wrap">
      <div className="t-body-s text-[var(--mute)]">
        Showing {total} of {totalAll} products
      </div>
      <div className="flex items-center gap-4 md:gap-5">
        <Link
          to="/size-guide"
          className="inline-flex items-center gap-2 text-[var(--ink)] no-underline text-[14px] font-medium border-b border-[var(--ink)] pb-0.5"
        >
